...
if (project && !document.querySelector('fluently-feedback')) {
  ...
  async function initialize() {
    ...
    style.textContent = `
      .menu{position:fixed;z-index:1;min-width:170px;max-width:calc(100vw - 16px);padding:5px;background:white;border:1px solid #ccd6e4;border-radius:9px;box-shadow:0 8px 28px #14243a30;pointer-events:auto;font:13px/1.5 system-ui,sans-serif}.menu button{display:block;width:100%;text-align:left;white-space:nowrap}.controls{display:flex;align-items:center;justify-content:flex-end;gap:8px;flex-wrap:wrap;padding:8px;background:white;border:1px solid #ccd6e4;border-radius:12px;box-shadow:0 5px 22px #14243a25;pointer-events:none;max-width:100%;position:relative;z-index:0;transform:translateY(68px);clip-path:inset(calc(100% - 56px) 0 0 calc(100% - 56px) round 28px);opacity:0;visibility:hidden;transition:transform .4s cubic-bezier(.22,.61,.36,1),clip-path .4s cubic-bezier(.22,.61,.36,1),opacity .3s ease,visibility 0s .4s}.controls[data-open="true"]{transform:translateY(0);clip-path:inset(0 round 12px);opacity:1;visibility:visible;pointer-events:auto;transition-delay:0s}
